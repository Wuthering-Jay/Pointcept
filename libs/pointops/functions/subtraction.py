...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        output = torch.zeros(n, nsample, c, device='cuda', dtype=torch.float)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros(n, c, device='cuda', dtype=torch.float)
        grad_input2 = torch.zeros(n, c, device='cuda', dtype=torch.float)
